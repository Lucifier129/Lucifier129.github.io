	...
	var Demo = _react2['default'].createClass({
	  ...
	  handleChange: function handleChange(_ref) {
	    ...
	    if (parseInt(value, 10) === Object.keys(photos).length - 1) {
	      ...
	      var w = Math.floor(Math.random() * 500 + 200);
	      var h = Math.floor(Math.random() * 500 + 200);
	      var hash = (Math.random() + '').slice(3);
	      this.setState({
	        photos: _extends({}, photos, (_extends2 = {}, _extends2['http://lorempixel.com/' + w + '/' + h + '/sports/a' + hash] = [w, h], _extends2))
	      });
	    }
	  },
	  ...
	  getStyles: function getStyles() {
	    ...
	    var currPhoto = _state.currPhoto;
	
	    var keys = Object.keys(photos);
	    var currKey = keys[currPhoto];
	    var _photos$currKey = photos[currKey];
	    var width = _photos$currKey[0];
	    var height = _photos$currKey[1];
	
	    var widths = keys.map(function (key) {
	      var _photos$key = photos[key];
	      var origW = _photos$key[0];
	      var origH = _photos$key[1];
	    ...
	    var configs = {};
	    keys.reduce(function (prevLeft, key, i) {
	      var _photos$key2 = photos[key];
	      var origW = _photos$key2[0];
	      var origH = _photos$key2[1];
	
	      configs[key] = {
	        left: _srcReactMotion.spring(prevLeft, [170, 26]),
	        height: _srcReactMotion.spring(height, [170, 26]),
	        width: _srcReactMotion.spring(height / origH * origW, [170, 26])
	      };
	      return prevLeft + widths[i];
	    }, offset);
	    configs.container = { height: _srcReactMotion.spring(height), width: _srcReactMotion.spring(width) };
	  ...
	  render: function render() {
	    ...
	    var photos = _state2.photos;
	    var currPhoto = _state2.currPhoto;
	    ...
	    return _react2['default'].createElement(
	      ...
	      _react2['default'].createElement(
	        'div',
	        null,
	        'When you scroll to the end, wait for the images to load.'
	      ),
	      ...
	      _react2['default'].createElement(
	        ...
	        function (_ref2) {
	          ...
	          return _react2['default'].createElement(
	            ...
	            _react2['default'].createElement(
	              'div',
	              { className: 'demo4-inner', style: container },
